...
public class ToolTipView extends TextView {
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        try {
            ...
            tooltipColor = Color.argb(255, random.nextInt() % 256, random.nextInt() % 256, random.nextInt() % 256);
            ...
            arrowPositioning = a.getInteger(R.styleable.ToolTipView_arrowLocation,
                    resources.getInteger(R.integer.tooltip_default_arrow_location));
            if (arrowPositioning == 0) {
                arrowLocation = new TopArrowLocation();
            } else if (arrowPositioning == 1) {
                arrowLocation = new BottomArrowLocation();
            } else if (arrowPositioning == 2) {
                arrowLocation = new RightArrowLocation();
            } else if (arrowPositioning == 3) {
                arrowLocation = new LeftArrowLocation();
//            }
            }
            arrowAlignment = ArrowAlignment.getAlignment(
                    a.getInteger(R.styleable.ToolTipView_arrowAlignment, resources.getInteger(
